...
- `docs/coordination/DECISIONS.md` 只追加，不覆盖或删除历史决策；每次追加必须同步在根目录 `DECISIONS.md` 加一行索引摘要（同一次工作内完成，防止索引断档）。
...
- 官方内容采用 cache-first：先读取 `reports/OFFICIAL_SYNC.md`、`docs/_starter_pack/03_OFFICIAL_SNAPSHOT_20260814.md`、已下载 notebook 和 `infra/bioinf-data-index/`；只有缓存缺失、疑似过期、用户明确要求或官方状态有变化风险时才重新抓取官网。重新抓取后必须记录日期、来源和变化。
...
- `docs/_starter_pack/` 默认只读；其中的官方契约优先于旧叙述。
