#!/usr/bin/env python3
"""G1-T3-R3 lineage dose: Gata4 = WT x (1 - dose), dose by cm_celltype lineage.
...
def main() -> int:
    ...
    ap.add_argument("--out-version", default="v0015_g0_t3_r3_lineagedose")
    args = ap.parse_args()
    ...
    diag: dict = {"task": TASK_ID, "seed": RUN_SEED,
                  "rule": "dose 1.0 cardiac-lineage cm states, 0.5 Peri/BW/Unknown (fixed upfront)"}
    t00 = time.time()
    ...
    a.uns["ve_g0_t3_r3"] = json.dumps({"atom_id": TASK_ID, "lane": "L_C_LINEAGEDOSE",
        "method": "lineage_dose_cardiac100", "parent": "v0009 rows",
    ...
    import subprocess
    mout = RUN / "metrics" / "scorer_L_C_LINEAGEDOSE.json"
    ...
    (RUN / "metrics" / "scorer_L_C_LINEAGEDOSE_slim.json").write_text(json.dumps(keep, indent=1))
    diag["local"] = keep
