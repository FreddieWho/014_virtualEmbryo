#!/usr/bin/env python3
"""G1-T3-R4 combo: Gata4 = WT x (1 - dose), dose by cm_celltype lineage.
...
def main() -> int:
    ...
    ap.add_argument("--out-version", default="v0016_g0_t3_r4_combo")
    args = ap.parse_args()
    ...
    a.uns["ve_g0_t3_r3"] = json.dumps({"atom_id": TASK_ID, "lane": "L_D_COMBO",
        "method": "lineage_dose_plus_17motif_prop", "parent": "v0009 rows",
    ...
    import subprocess
    mout = RUN / "metrics" / "scorer_L_D_COMBO.json"
    ...
    (RUN / "metrics" / "scorer_L_D_COMBO_slim.json").write_text(json.dumps(keep, indent=1))
    diag["local"] = keep
