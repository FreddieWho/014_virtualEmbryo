#!/usr/bin/env python3
"""G1-T2-R3 spatially-smoothed shift field.
...
def strict_deltas(Xp, tp, Xb, tb, panel_len: int):
    """Return {type: strict damp=1.0 delta}. Pure function (deterministic)."""
    out = {}
    info = []
    for t in sorted(set(tb.tolist())):
        ...
        if (tp == t).sum() == 0:
            ...
            info.append({"type": t, "n_e95": int(len(mb)), "n_e85": 0})
            continue
        mp = Xp[tp == t]
        out[t] = (mb.mean(axis=0) - mp.mean(axis=0)).astype(np.float32)
        info.append({"type": t, "n_e95": int(len(mb)), "n_e85": int(len(mp))})
...
def main() -> int:
    ...
    ap.add_argument("--out-version", default="v0012_g0_t2_r3_spatial")
    ap.add_argument("--lane", default="L1_SPATIAL")
    ...
    diag: dict = {"task": TASK_ID, "seed": RUN_SEED, "smooth": f"knn{KNN} delta-field", "lane": lane}
    t00 = time.time()
    ...
    deltas, info = strict_deltas(Xp, tp, Xb, tb, len(panel))
    info.to_csv(RUN / "intermediates" / "delta_table.tsv", sep="\t", index=False)
    # Determinism gate: recompute, compare.
    deltas2, _ = strict_deltas(Xp, tp, Xb, tb, len(panel))
    assert all(np.array_equal(deltas[t], deltas2[t]) for t in deltas)
    diag["rerun_deltas_identical"] = True

    # Spatial smoothing of the ADDED delta field (base expression untouched).
    # kNN (k=15, fixed) over source-cell spatial_3D within the 25179-row set.
    from sklearn.neighbors import NearestNeighbors
    Csrc = np.asarray(base.obsm["spatial_3D"])[src_idx].astype(np.float64)
    assert Csrc.shape == (N_CELLS, 3) and np.isfinite(Csrc).all()
    nbrs = NearestNeighbors(n_neighbors=KNN, algorithm="kd_tree").fit(Csrc)
    _, knn_idx = nbrs.kneighbors(Csrc)
    t_src = tb[src_idx]
    Dmat = np.stack([deltas[str(t)] for t in t_src]).astype(np.float32)
    Dsmooth = Dmat[knn_idx].mean(axis=1).astype(np.float32)
    diag["knn_k"] = KNN
    diag["delta_shift_l2"] = float(np.linalg.norm(Dsmooth - Dmat) / np.linalg.norm(Dmat))
    # Determinism gate: neighbor search is exact kd_tree.
    _, knn_idx2 = NearestNeighbors(n_neighbors=KNN, algorithm="kd_tree").fit(Csrc).kneighbors(Csrc)
    assert np.array_equal(knn_idx, knn_idx2)
    diag["rerun_knn_identical"] = True

    Xout = np.empty((N_CELLS, len(panel)), dtype=np.float32)
    for j, si in enumerate(src_idx):
        Xout[j] = Xb[si] + Dsmooth[j]
    ...
    a.uns["ve_g0_t2_r3"] = json.dumps({"atom_id": TASK_ID, "lane": lane,
        "method": "spatial_knn15_delta_smooth", "parent": "V0001 recipe",
    ...
    print(json.dumps({"beats_baseline": diag["beats_baseline"], "local": keep,
                      "delta_shift_l2": diag["delta_shift_l2"]}, indent=1), flush=True)
    return 0
