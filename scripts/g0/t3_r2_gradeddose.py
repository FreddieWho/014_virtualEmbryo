#!/usr/bin/env python3
"""G1-T3-R2 graded dose: Gata4 = WT x (1 - dose), dose by WT-expression tertile.
...
dose tertiles {0.5, 0.75, 1.0} by WT Gata4 rank among the 7449 rows. Other 499 genes = parent.
...
def main() -> int:
    ...
    ap.add_argument("--out-version", default="v0014_g0_t3_r2_gradeddose")
    args = ap.parse_args()
    ...
    diag: dict = {"task": TASK_ID, "seed": RUN_SEED,
                  "rule": "dose {0.5,0.75,1.0} by WT-Gata4 tertile among 7449 rows"}
    t00 = time.time()
    ...
    gw = Xout[:, gi].copy()
    e1, e2 = np.quantile(gw, [1 / 3, 2 / 3])
    bins = np.clip(np.digitize(gw, [e1, e2]), 0, 2)
    doses = np.array([0.5, 0.75, 1.0], dtype=np.float32)[bins]
    pd.DataFrame({"dose": doses}).describe().to_csv(RUN / "intermediates" / "dose_summary.tsv", sep="\t")
    diag["dose_edges"] = [float(e1), float(e2)]
    diag["dose_counts"] = [int((bins == j).sum()) for j in range(3)]
    # Determinism gate: ranks/quantiles are pure functions of fixed inputs.
    gw_check = np.asarray((src.X[s_idx][:, gi].toarray().ravel()
                           if sparse.issparse(src.X[s_idx]) else src.X[s_idx][:, gi]),
                          dtype=np.float32)
    assert np.array_equal(gw_check, gw), "WT Gata4 pull not deterministic"
    ...
    a.uns["ve_g0_t3_r2"] = json.dumps({"atom_id": TASK_ID, "lane": "L_B_GRADEDDOSE",
        "method": "graded_dose_tert075", "parent": "v0009 rows",
    ...
    import subprocess
    mout = RUN / "metrics" / "scorer_L_B_GRADEDDOSE.json"
    ...
    (RUN / "metrics" / "scorer_L_B_GRADEDDOSE_slim.json").write_text(json.dumps(keep, indent=1))
    diag["local"] = keep
