...
- [x] 路线6准入与输入准备：2026-09-21新规则生效，26条件重新审查通过，5454×500矩阵及WT图/特征已完成；可启动signed训练，不再统一要求书面确认。预测器训练尚未执行。
...
- [x] R5 v0032/v0033总分及五子分已按用户回报登记（2026-09-21）；两者并列父版本，无总分增益。submission ID/time未提供。见 `reports/SERVER_SCORE_REGISTRY.md#t3-next-r5-score-return-20260921`。
- 已替代（2026-09-21）：旧的普遍书面确认与shape-only要求取消；本次26个条件按实际来源上下文重新审查通过。未取得确认函，也不伪称已取得。

- [x] 2026-09-20：登记R1/R2四组服务器总分及20个子分，核对artifact SHA；best不变。
- [x] 2026-09-21按规则评审修改生效政策与R6用途门，26条件重新审查与数据准备完成；证据 `reports/t3_r56_launch_20260921/REPORT.md`。

- [x] 2026-09-21 R6首次训练、留出评估和目标推断完成；输出负值门槛失败，无候选。见 `reports/t3_r6_execution_20260921/REPORT.md`。
- [x] R6非负输出修复已完成；跨成人→胚胎泛化与基因特异预测优势仍未建立，原失败记录保留。

### T3全路线复核后的整合建议（2026-09-21，未执行新分支）

- [x] 复核本轮R1–R6及历史技术家族，补充只读数值诊断；报告 `reports/t3_route_review_20260921/REPORT.md`。
- [x] P1 R6：非负输出、样本内NTC、强对照和五基因折/样本迁移已完成；图优势未成立，v0034/v0035仅待服务器探索。
- [x] P1 R3：非负机制、四空间块及零输入支持诊断完成，v0036/v0037待评分；因果支持仍未知。
- [x] P1 R4：残差映射20组评价通过，panel-only/中介反事实应用校准斜率完成，v0040/v0041待评分；v0038/v0039撤回。
- [ ] P2 整合R1结构保护、R5有效信号链及旧图/rank先验；不原样重跑R2整细胞替换或全表达平滑。

- [ ] 上传 `deliveries/r634fix__t3__upload__20260921.zip` 六个修复候选并回填总分/五子分；R5旧分数已登记。
- [x] R1结构组件对修复R6两候选均NO_OP；不建立重复候选。R5扩链未执行。

### T1新六路线（2026-09-21，仅提案）

- [x] 完成历史路线/服务器结果/官方输入metadata核对，准备 `reports/T1_NEXT_ROUTES_20260921.md`。
- [ ] R2：残差保留decoder，先验证零变化恒等与全基因方差保留。
- [ ] R1：分布零率/阳性分位数外推，与固定零支持对照。
- [ ] R5：固定逐基因边际，评价阶段依赖演化与置乱方向。
- [ ] R3：稳定有界仿射动力学，比较对角与低秩交互。
- [ ] R4：模块运输代价与正交残差保留，最终全32,285基因评价。
- [ ] R6：先审查T1早期多阶段来源、去重、生成任务专属许可，再做时间表示迁移；当前数据准备未执行。
