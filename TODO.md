...
- [ ] R6后继：设计保证非负的表达量依赖输出，保留本轮失败记录；成人→胚胎迁移及样本批次敏感性仍待验证。未自动调参/放宽门槛。

### T3全路线复核后的整合建议（2026-09-21，未执行新分支）

- [x] 复核本轮R1–R6及历史技术家族，补充只读数值诊断；报告 `reports/t3_route_review_20260921/REPORT.md`。
- [ ] P1 R6：非负、表达量依赖输出；样本内NTC；平均响应/无图/置乱图强对照与新的外层评估。不能仅缩小alpha。
- [ ] P1 R3：非负机制残差、四空间块外评价、零输入支持；保留原失败receipt。
- [ ] P1 R4：类型内残差映射与跨平台尺度，先胜过类型均值，再做panel-only/中介增量。
