...
- 成熟度：完整来源数据上的固定交叉评价。R6改为非负倍率后，GCN源域pooled MSE优于无变化但差于平均响应、平均倍率和置乱图，不能归因图结构增益；双向样本迁移也未胜过无变化。新旧评价折与目标表示不同，不以误差变化证明进步。
- 成熟度：WT内空间块外工程验证。R3线性/样条4块均略优于类型均值，R4校准映射20组均优于类型均值；均不构成Gata4 KO验证。
- 六候选非负且原零值支持不变，但没有新激活建模；R1叠加新R6为NO_OP。证据 `reports/t3_repairs_20260921/REPORT.md` 及所引run；科学限制blocks_submission: false，服务器未评分。
